...
if (Meteor.isServer) {
  Meteor.publish('secret-santa.matches', function (userId) {
  ...
  Meteor.publish('secret-santa.messages', function (matchId, userId) {
    const match = Matches.findOne({_id: new Mongo.ObjectID(matchId)});
    const user = Meteor.users.findOne({_id: userId});
  ...
  Meteor.publish('secret-santa.match', function (groupId, discordUserId) {
    const matches = Matches.find({
  ...
  Meteor.publish('secret-santa.matches.shippingInfo', function (groupId) {
    return Matches.find({
      groupId
    }, {
      fields: {
