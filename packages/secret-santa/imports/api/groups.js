...
if (Meteor.isServer) {
  Meteor.publish('secret-santa.groups', function (userId) {
  ...
  Meteor.publish('secret-santa.group', function (id) {
  ...
  Meteor.publish('secret-santa.groups.ending', function (userId) {
    const user = Meteor.users.findOne({_id: userId});
